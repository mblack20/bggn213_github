...
Sample data to do clustering analysis where we know what the answer should be. Use the 'rnorm()' function.
...
Unlike "kmeans()', we can't just put in our data as input. First you need a distance matrix from the data
...
To get the main clustering result (membership vector, which cluster each point belongs to), you can cut the tree at a given height. To do this, use 'cutree()'
...
Make the main result figure, called a "PC plot" (or score plot, or ordination plot, or PC1 vs PC2 plot), 
```{r}
pca$x
